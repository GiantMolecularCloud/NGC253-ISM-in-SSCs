...
def create_model_molfit(num, SSC):
    ...
    fit   = escape_fname(os.path.join(tempdir,'SSC_'+str(SSC['no']),'run_'+str(num),'fit','results','molecules__LM__call_1.out.molfit'))
    model = escape_fname(os.path.join(tempdir,'SSC_'+str(SSC['no']),'run_'+str(num),'model','molecules.molfit'))
    ...
    tempfiles[SSC['num']][str(num)]['model'] = {'molfit': model}
...
def model_spectrum_XCLASS(num, SSC):
    ...
    modelpath  = tempfiles[SSC['num']][str(num)]['model']['molfit']
    savepath   = escape_fname(os.path.join(tempdir,'SSC_'+str(SSC['no']),'run_'+str(num),'model','XCLASS_model.py'))
    resultpath = escape_fname(os.path.join(tempdir,'SSC_'+str(SSC['no']),'run_'+str(num),'model','results'))
    specpath   = escape_fname(os.path.join(tempdir,'SSC_'+str(SSC['no']),'run_'+str(num),'model','spectrum.pickle'))
    energypath = escape_fname(os.path.join(tempdir,'SSC_'+str(SSC['no']),'run_'+str(num),'model','energy.pickle'))
...
""")
    tempfiles[SSC['num']][str(num)]['model']['file']     = savepath
    tempfiles[SSC['num']][str(num)]['model']['spectrum'] = specpath
    tempfiles[SSC['num']][str(num)]['model']['energy']   = energypath
    tempfiles[SSC['num']][str(num)]['model']['tau']      = taupath
...
def CASA_command(SSC):
    XCL_files = [tempfiles[SSC['num']][str(num)]['model']['file'] for num in nums]
    Xcommand = f"""
...
def plot_combined_variation(nums, SSC, band):
    ...
    def get_spectra(nums, SSC, band):
        spectra = [ np.genfromtxt(tempfiles[SSC['num']][str(num)]['dat'][band], dtype=None) for num in nums ]
    ...
    def get_models(nums, SSC, band):
        models = [ tempfiles[SSC['num']][str(num)]['model']['spectrum'] for num in nums ]
        spectra = []
        for model in models:
    ...
    def save_figure(fig, band):
        savepath = escape_fname(os.path.join(plotdir, '05.temperatures', 'spectra', 'SSC_'+str(SSC['no'])+'.'+band+'.spectrum.pdf'))
