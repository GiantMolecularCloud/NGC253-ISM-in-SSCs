...
def parse_molfit(SSC):
    ...
    def load_molfit(num, SSC):
        file = Xfiles[SSC['num']][str(num)]['model']['molfit']
...
final_data = {SSC['num']: parse_molfit(SSC) for SSC in SSCs}
get_opacity(final_data)
fnpickle(final_data, os.path.join(Xfinaldir, 'data.pickle'))
...
def plot_parameters(SSC):
    ...
    for q,ylab,ymin,ymax in [['column density','N [cm$^{-2}$]',1e11,1e21],['linewidth','$\sigma$ [km\,s$^{-1}$]',0,90],['velocity','v [km\,s$^{-1}$]',-50,50]]:
        ...
        fitted_species = final_data[SSC['num']].keys()
        colors = [plt.cm.inferno(i/(len(fitted_species)+1)) for i,_ in enumerate(fitted_species)]

        for idx,(specie,c) in enumerate(zip(fitted_species, colors)):
            for q_all in final_data[SSC['num']][specie][q]['all']:
        ...
        savepath = os.path.join(plotdir, '04.XCLASS_final', 'parameters', 'SSC_'+SSC['num']+'.'+q+'.pdf')
...
def plot_fit_hist(SSC, spx):
    ...
    def save_figure(fig, spx):
        savepath = escape_fname(os.path.join(plotdir, '04.XCLASS_final', 'histograms','SSC_'+SSC['num']+'.'+spx+'.histogram.pdf'))
        os.system('mkdir -p '+os.path.dirname(savepath))
        fig.savefig(savepath, dpi=300, bbox_inches='tight')

    # temperatures     = final_data[SSC['num']][spx]['temperature']['all']
    column_densities = final_data[SSC['num']][spx]['column density']['all']
    linewidths       = final_data[SSC['num']][spx]['linewidth']['all']
    velocities       = final_data[SSC['num']][spx]['velocity']['all']
    opacities        = final_data[SSC['num']][spx]['integrated opacity']['all']
...
for SSC in tqdm(SSCs):
    for spx in final_data[SSC['num']].keys():
        try:
