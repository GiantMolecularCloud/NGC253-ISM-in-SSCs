...
def plot_combined_variation(nums, SSC, band):
    ...
    def get_spectra(nums, SSC, band):
        spectra = [ np.genfromtxt(Xfiles[SSC['num']][str(num)]['dat'][band]['file'], dtype=None) for num in nums ]
    ...
    def get_models(nums, SSC, band):
        models = [ Xfiles[SSC['num']][str(num)]['model']['spectrum'] for num in nums ]
        spectra = []
        for model in models:
    ...
    def save_figure(fig, band):
        savepath = escape_fname(os.path.join(plotdir, '04.XCLASS_final', 'spectra', 'SSC_'+str(SSC['no'])+'.'+band+'.spectrum.pdf'))
