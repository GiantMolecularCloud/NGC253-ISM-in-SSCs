...
def parse_molfit(SSC):
    ...
    def load_molfit(num, SSC):
        file = vibfiles[SSC['num']][str(num)]['model']['molfit']
...
vibrational_data = {SSC['num']: parse_molfit(SSC) for SSC in SSCs}
get_opacity(vibrational_data)
fnpickle(vibrational_data, os.path.join(vibdir, 'vibrational_data.pickle'))
...
def plot_fit_hist(SSC, spx):
    ...
    def save_figure(fig, spx):
        savepath = escape_fname(os.path.join(plotdir, '06.vibrations', 'histograms','SSC_'+SSC['num']+'.'+spx+'.histogram.pdf'))
        os.system('mkdir -p '+os.path.dirname(savepath))
        fig.savefig(savepath, dpi=300, bbox_inches='tight')

    # temperatures     = vibrational_data[SSC['num']][spx]['temperature']['all']
    column_densities = vibrational_data[SSC['num']][spx]['column density']['all']
    linewidths       = vibrational_data[SSC['num']][spx]['linewidth']['all']
    velocities       = vibrational_data[SSC['num']][spx]['velocity']['all']
    opacities        = vibrational_data[SSC['num']][spx]['integrated opacity']['all']
...
for SSC in tqdm(SSCs):
    for spx in vibrational_species:
        if spx in vibrational_data[SSC['num']].keys():
